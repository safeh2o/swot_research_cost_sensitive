...
for z in range (0, len(test_lengths)):
    ...
    for i in np.arange(0,2.05,0.05):
        upper_b = np.around(i + 0.05, decimals=2)
        lower_b = np.around(i, decimals=2)
        capture_mean = np.append(capture_mean,np.sum(capture_all*np.less(X_test,upper_b)*np.greater_equal(X_test,lower_b))/(np.sum(np.less(X_test,upper_b)*np.greater_equal(X_test,lower_b))))
        if np.sum(np.less(Y_test,0.2)*np.sum(np.less(X_test,upper_b)*np.greater_equal(X_test,lower_b)))>0:
            capture_20_mean = np.append(capture_20_mean,np.sum(capture_all_20*np.less(X_test,upper_b)*np.greater_equal(X_test,lower_b))/np.sum(np.less(Y_test,0.2)*(np.less(X_test,upper_b)*np.greater_equal(X_test,lower_b))))

    capture_mean=np.nanmean(capture_mean)
    capture_20_mean=np.nanmean(capture_20_mean)
    ...
    store_mean_capture = np.append(store_mean_capture,capture_mean)
    store_capture_20 = np.append(store_capture_20,capture_all_20_sum/length_20)
    store_mean_capture_20 = np.append(store_mean_capture_20,capture_20_mean)
    ...
    store_Reli = np.append(store_Reli, Reli)
    ...
    #ax1.set_xlim([0,np.max(X_test)])
    #ax1.set_ylim([0,np.max(X_test)])
...
df=pd.DataFrame({'Experiment':exp_names,'Capture Percent':store_capture,'Mean Capture Percent':store_mean_capture,'Percent Capture (sub02)':store_capture_20,'Mean Percent Capture (sub02)':store_mean_capture_20,'CI Reliability Sum of Squares':store_CI_sumsquares,'CI Reliability Sum of Squares (sub02)':store_CI_20_sumsquares,'Delta':store_delta,'Delta (sub02)':store_delta_20,'CRPS':store_CRPS,'Reli':store_Reli})
